...
lock_pool()
{
	//g_threadDirectFuncs.criticalSectionEnter(&g_poolLock);
}
...
unlock_pool()
{
	//g_threadDirectFuncs.criticalSectionExit(&g_poolLock);
}
...
void SSharedBuffer::IncUsers() const
{
	if ((m_users&B_STATIC_USERS) == 0) {
		atomic_add(&m_users, 1<<B_BUFFER_USERS_SHIFT);
		return;
	}
...
void SSharedBuffer::DecUsers() const
{
	...
	if ((users&B_STATIC_USERS) == 0) {
		int32_t prev = atomic_add(&m_users, -(1<<B_BUFFER_USERS_SHIFT))>>B_BUFFER_USERS_SHIFT;
	...
	if ((pooled->m_users&B_POOLED_USERS) == 0) {
		...
		if (find_in_pool(this, &pos)) {
			...
			if (add_to_pool(pooled, pos)) {
				atomic_or(&m_users, B_POOLED_USERS);
			}
		}
