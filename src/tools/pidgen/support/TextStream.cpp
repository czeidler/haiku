...
#define B_NS2US(ns)                             ((int64)(ns) / 1000)
#define B_NS2MS(ns)                             ((int64)(ns) / 1000000)
#define B_NS2S(ns)                              ((int64)(ns) / 1000000000)
...
BTextOutput::BTextOutput(const sptr<BVFSFile>& stream, uint32_t flags)
...
BTextOutput::BTextOutput(BVFSFile *This, uint32_t flags)
...
{
	return ((const BVFSFile*)m_stream)->AsBinder();
}*/
...
void BTextOutput::MoveIndent(int32 delta)
{
#if SUPPORTS_TEXT_STREAM
	style_state* style = Style();
	
	//g_threadDirectFuncs.atomicAdd32(&style->indent, delta);
	atomic_add(&style->indent, delta);
...
SString SStatus::AsString() const
{
	...
	if ((m_status&0x40000000) != 0) {
		...
		const char* str = strerror(-m_status);
		#else
...
SHexDump& SHexDump::SetSingleLineCutoff(int32 bytes)
{
...
int32 SHexDump::SingleLineCutoff() const	{ return fSingleLineCutoff; }
