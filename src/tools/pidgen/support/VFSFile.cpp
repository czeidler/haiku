...
#define checkpoint \
cerr << "(" << this << ") -- " << __FILE__ << ":" << __LINE__ << endl;
...
ssize_t BVFSFile::ReadV(const struct iovec *vector, ssize_t count)
{
	return ReadAtV(m_pos, vector, count);
}

ssize_t BVFSFile::WriteV(const struct iovec *vector, ssize_t count)
{
	return WriteAtV(m_pos, vector, count);
}
