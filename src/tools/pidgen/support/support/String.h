...
B_CONST_STRING_VALUE_SMALL(B_BINDER_EMPTY_STRING, "", );
#else
...
public:
	...
	#if TARGET_HOST == TARGET_HOST_BEOS
	status_t			PathSetTo(const SString &dir, const SString &leaf = B_BINDER_EMPTY_STRING,
...
//clemens SValue BArrayAsValue(const SString* from, size_t count);
//clemens status_t BArrayConstruct(SString* to, const SValue& value, size_t count);
