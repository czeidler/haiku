...
//class SValue;
typedef int64 nsecs;
typedef int64 nsecs_t;
#define LIBBE_BOOTSTRAP 1
...
/*!	@todo This should probably be changed to not derive from
	...
	anyway.
*/
class ITextOutput : public SAtom
{
public:
...
_IMPEXP_SUPPORT const sptr<ITextOutput>& operator<<(const sptr<ITextOutput>& io, uint64);
_IMPEXP_SUPPORT const sptr<ITextOutput>& operator<<(const sptr<ITextOutput>& io, int64);
...
//!	Bytes size container for formatting to an ITextOutput.
class SSize
{
public:
	SSize(size_t s);
...
public:
	...
	SHexDump& SetSingleLineCutoff(int32 bytes);
...
inline SCharCode::SCharCode(uint32 c)
	:val(c)
{
}
