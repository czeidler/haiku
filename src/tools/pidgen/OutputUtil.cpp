...
static SString ToStrongPointer(const SString interfaceName)
{
	SString output("BReference<");
...
static SString ToWeakPointer(const SString interfaceName)
{
	SString output("BWeakReference<");
	output += interfaceName;
	output += ">";
	return output;
}
...
TypeToCPPType(const InterfaceRec* rec, const sptr<IDLType>& obj, bool asConst)
{
	...
	if (cpptype==NULL) {
		...
		if (cpptype=="sptr") {
			...
			InterfaceRec* iface=FindInterface(obj->GetIface());
			if (obj->HasAttribute(kWeak) == true)
				cpptype = ToWeakPointer(iface->ID());
			else
				cpptype = ToStrongPointer(iface->ID());
